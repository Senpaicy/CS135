...
(define (CRT-exists? cong-sys)
  (define x (help cong-sys))
  (compare x x))

(define (help x)
    (cond [(equal? x '()) '()]
          [else (cons (cadar x) (help (cdr x)))]))

(define (compare x y)
  (cond [(equal? (cdr x) '()) #t]
        [(equal? y '()) (compare (cdr x) x)]
        [(equal? (car x) (car y)) (compare x (cdr y))]
        [(equal? (euclid-gcd (car x)(car y)) 1) (compare x (cdr y))]
        [else #f]))
...
(define (mulhelper a b c)
(cond [(equal? (modulo (* a c) b) 1) c]
      [else (mulhelper a b (- c 1))]))
...
(define (m cong-sys)
  (cond [(null? cong-sys) 1]
        [else (* (mul-inv (caar cong-sys) (cadar cong-sys)) (m cong-sys))]))
...
(define (help2 x y)
  (cond [(null? x) '(0)]
        [else (append
               (list
                 (* (* (/ y (cadar x))(caar x))(mul-inv (/ y (cadar x)) (cadar x))))
                 (help2 (cdr x) y))]))
...
(define (CRT cong-sys)
  (define x (m cong-sys))
  (cond [(CRT-exists? cong-sys) (modulo (CRT-helper cong-sys x) x)]
        [else -1]))