...
(define (generate p q e)
  ...
  (append (list(list m e)) (list(list m d))))
...
(define (mod-exp b e m)
  (cond [(equal? b 0) 0]
        [(equal? e 0) 1]
        [(equal? m 0) "use a different number."]
        [else (modulo (expt b e) m)]))
...
(define (encrypthelp plain key)
  (cond [(null? plain) '()]
        [else (append (list (mod-exp (char->integer(car plain))(cadr key)(car key)))
                      (encrypthelp (cdr plain) key))]))
...
(define (decrypthelp ciphertext key)
  (cond [(null? ciphertext) '()]
        [else (append (list(integer->char (mod-exp (car ciphertext) (cadr key) (car key)))) (decrypthelp (cdr ciphertext) key))]))
