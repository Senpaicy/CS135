...
(define (get-vertices G)
  (cond [(equal? G empty) empty]
        [else (usort (cons(caar G)(cons(cadar G) (get-vertices (cdr G)))))]))
...
(define (degree G v)
  (cond [(equal? G empty) 0]
        [(and (equal? v (caar G)) (equal? v (cadar G)))
           (+ 2 (degree (cdr G) v))]
        [(or (equal? v (caar G))(equal? v (cadar G)))
            (+ 1 (degree (cdr G) v))]
        [else (degree (cdr G) v)]))
...
;; 1. go through vertex list
;; 2. get degree of the vertex
;; 3. add them up until list is empty
;; 4. mod that
;; 5. and return true if theres no remainder
;;
;; Examples:
;;   (all-even? C6) -> #t
;;   (all-even? K4) -> #f
...
(define (all-even? G)
  (check (help G (get-vertices G))))


(define (check G)
  (cond [(null? G) #t]
        [(not (equal? 0 (modulo (car G) 2))) #f]
        [else (check (cdr G))]))

(define (help G V)
  (cond [(null? V) '()]
        [else (append (list (degree G (car V)))(help G (cdr V)))]))
